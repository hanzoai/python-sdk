...
def get_embedding_service() -> EmbeddingService:
    ...
    if _embedding_service is None:
        ...
        if settings.db_backend == "lancedb" and LANCEDB_AVAILABLE:
            ...
            # Create a minimal embedding service that generates dummy embeddings
            # This allows the system to work without heavy embedding dependencies
            logger.warning("Using minimal embedding service (dummy embeddings)")
            _embedding_service = MinimalEmbeddingService()
    return _embedding_service


class MinimalEmbeddingService:
    """Minimal embedding service that generates dummy embeddings for basic functionality."""

    def __init__(self):
        self.model_name = "minimal"

    def embed_text(self, text: str | list[str]) -> list[list[float]]:
        """Generate dummy embeddings for text."""
        if isinstance(text, str):
            text = [text]

        # Generate simple dummy embeddings based on text length and content
        embeddings = []
        for t in text:
