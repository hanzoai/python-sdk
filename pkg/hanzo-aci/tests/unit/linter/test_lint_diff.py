...
from dev_aci.utils.diff import get_diff, parse_diff
...
def test_lint_file_diff_ignore_existing_errors(tmp_path):
    ...
    In other words, we don't care about existing linting errors. Although they might be
...
def test_lint_file_diff_catch_new_errors_in_edits(tmp_path):
    """Make sure we catch new linting errors in our edit chunk.

    At the same time, ignore old linting errors (in this case, the old linting error is
    a false positive).
    """
    content = """def some_valid_but_weird_function():
    # this function is legitimate, yet static analysis tools like flake8
...
def test_lint_file_diff_catch_new_errors_outside_edits(tmp_path):
    ...
    Even though the error itself is not in the edit chunk.
    """
