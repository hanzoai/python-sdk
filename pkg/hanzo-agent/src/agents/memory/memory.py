...
class Memory:
    ...
    async def reflect(
        ...
        if not recent:
            ...
            memory_text = "\n".join([f"- [{m.type.value}] {m.content}" for m in recent])
    ...
    async def _compress_memories(self) -> None:
        ...
        for memory in memories:
            ...
            score = memory.importance * 0.5 + recency_score * 0.3 + access_score * 0.2
    ...
    def create_agent_wrapper(self, agent: Agent[TContext]) -> Agent[TContext]:
        ...
        async def memory_instructions(
            ctx: RunContextWrapper[TContext], agent: Agent[TContext]
        ) -> str:
