...
class AgentNetwork:
    ...
    def add_agent(
        ...
        *,
        ...
        logger.debug(
            f"Added agent '{agent.name}' to network with capabilities: {node.capabilities}"
        )
        ...
        if starting_agent:
            if starting_agent not in self.nodes:
                raise AgentsException(
                    f"Starting agent '{starting_agent}' not found in network"
                )
            ...
            logger.debug(
                f"Router selected starting agent: {decision.selected_agent} (confidence: {decision.confidence})"
            )
...
class NetworkContextWrapper(RunContextWrapper):
    ...
    async def set_state(
        self, key: str, value: Any, namespace: str | None = None
    ) -> None:
        """Set a value in the shared state store."""
        await self.state_store.set(key, value, namespace)

    async def update_state(
        self, key: str, updater: Callable[[Any], Any], namespace: str | None = None
    ) -> Any:
...
class NetworkHooks(RunHooks):
    ...
    async def on_agent_start(
        ...
        if agent.name in self.network.nodes:
            ...
            logger.debug(
                f"Network agent '{agent.name}' started (execution #{node.execution_count})"
            )
