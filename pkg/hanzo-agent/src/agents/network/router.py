...
class Router(ABC):
    ...
    def update_agent_info(
        self, agent_name: str, capabilities: List[str], metadata: Dict[str, Any]
    ) -> None:
...
class LoadBalancingRouter(Router):
    ...
    async def route(
        ...
        if self.strategy == "round_robin":
            ...
            loads = [
                (agent, self.agent_loads.get(agent, 0)) for agent in available_agents
            ]
    ...
    async def route(
        ...
        for decision, weight in decisions:
            if decision.selected_agent:
                ...
                agent_scores[decision.selected_agent] = (
                    agent_scores.get(decision.selected_agent, 0) + score
                )
