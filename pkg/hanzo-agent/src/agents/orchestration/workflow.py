...
class Workflow:
    ...
    def get_execution_order(self) -> List[List[str]]:
        ...
        visited = set()
        in_degree = {
            step_id: len(step.depends_on) for step_id, step in self.steps.items()
        }
...
class Step:
    ...
    def agent(agent_name: str, prompt: str, **kwargs) -> WorkflowStep:
        ...
        """
        config = {"agent_name": agent_name, "prompt": prompt, **kwargs}
        return WorkflowStep(
            name=f"Run {agent_name}", type=StepType.AGENT, config=config
        )
    ...
    def parallel(steps: List[WorkflowStep]) -> WorkflowStep:
        ...
        """
        return WorkflowStep(
            name="Parallel Steps", type=StepType.PARALLEL, config={"steps": steps}
        )
    ...
    def loop(
        ...
        return WorkflowStep(
            name="Loop",
            type=StepType.LOOP,
            config={"over": over, "body": body.id, "max_iterations": max_iterations},
        )
    ...
    def transform(transform: Callable[[Any], Any]) -> WorkflowStep:
        ...
        """
        return WorkflowStep(
            name="Transform", type=StepType.TRANSFORM, config={"transform": transform}
        )
    ...
    def wait(duration: float) -> WorkflowStep:
        ...
        """
        return WorkflowStep(
            name=f"Wait {duration}s", type=StepType.WAIT, config={"duration": duration}
        )
