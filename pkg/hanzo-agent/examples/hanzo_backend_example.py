...
class HanzoModelProvider(ModelProvider):
    ...
    def get_model(self, model_name: str | None) -> Model:
        ...
        return OpenAIChatCompletionsModel(
            model=model_name or "gpt-3.5-turbo", openai_client=self.client
        )
...
async def multi_model_example():
    ...
    result = await Runner.run(
        ...
        run_config=RunConfig(
            model_provider=hanzo_provider, model="gpt-3.5-turbo"  # Fast, cost-effective
        ),
    )
...
async def main():
    ...
    try:
        ...
        print("\nTroubleshooting:")
        print(
            "1. Ensure Hanzo Router is running: cd /path/to/services && make start-router"
        )
