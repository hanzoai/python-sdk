...
class TestMultiAgentNetworks:
    ...
    def test_create_network(self, math_agent, science_agent):
        ...
        network = create_network(
            agents=[math_agent, science_agent], name="Test Network"
        )
    ...
    def test_semantic_router(self, math_agent, science_agent):
        ...
        network = create_network(agents=[math_agent, science_agent], router=router)
...
class TestStateManagement:
    ...
    async def test_in_memory_state_store(self):
        ...
        # Test update
        result = await store.update("counter", lambda x: (x or 0) + 1)
        assert result == 1
    ...
    async def test_state_sharing_in_network(self, math_agent, science_agent):
        ...
        store = InMemoryStateStore()
        network = create_network(agents=[math_agent, science_agent], state_store=store)
...
class TestMemorySystem:
    ...
    async def test_memory_compression(self):
        ...
        for i in range(5):
            await memory.add(content=f"Memory {i}", type=MemoryType.CONVERSATION)
...
class TestWorkflowOrchestration:
    ...
    def test_conditional_steps(self):
        ...
        conditional = Step.conditional(
            condition=check_condition, if_true=true_step, if_false=false_step
        )
    ...
    def test_loop_steps(self):
        ...
        loop = Step.loop(
            over="items", body=body_step, max_iterations=10  # Path to array in context
        )
...
class TestErrorHandling:
    ...
    async def test_state_store_error_handling(self):
        ...
        result = await store.update("new_key", lambda x: (x or 0) + 1)
        assert result == 1
...
class TestIntegration:
    ...
    async def test_network_with_state_and_memory(self, math_agent, science_agent):
        ...
        # Create network
        network = create_network(
            agents=[math_agent, science_agent], state_store=state_store
        )
        ...
        await math_memory.add(
            content="User prefers simple explanations", type=MemoryType.FACT
        )

        await science_memory.add(
            content="User is interested in physics", type=MemoryType.FACT
        )
    ...
    def test_workflow_with_network(self, math_agent, science_agent):
        ...
        # Create network
        network = create_network(agents=[math_agent, science_agent])

        # Create workflow
        workflow = Workflow(name="Math and Science Workflow")

        # Add steps using network agents
        math_step = workflow.add_step(Step.agent("MathExpert", "Calculate 2+2"))
