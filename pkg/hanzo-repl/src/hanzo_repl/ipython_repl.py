...
from hanzo_mcp.tools import *  # noqa: F403 - Import all tools for direct REPL access
...
class HanzoMagics(Magics):
    ...
    @cell_magic
    def ai(self, line, cell):  # noqa: ARG002 - IPython magic method signature
        """Multi-line AI chat."""
        message = cell.strip()
        if not message:
            return
    ...
    @line_magic
    def tools(self, line):  # noqa: ARG002 - IPython magic method signature
    ...
    def tool(self, line):
        ...
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self.repl.execute_tool(tool_name, args))
    ...
    def model(self, line):
        ...
        if not line:
            self.repl.llm_client.get_model_info()
        else:
