...
""")
        ...
        # Test fmt tool
        fmt_result = await tools.fmt(target_spec, FmtArgs(opts={"local_prefix": "github.com/luxfi"}))
        print(f"  fmt result: {'✅' if fmt_result.ok else '❌'} {fmt_result.language_used}")
        ...
        print(f"  edit result: {'✅' if edit_result.ok else '❌'} {edit_result.backend_used}")
        ...
        print(f"  guard result: {'✅' if guard_result.ok else '❌'} violations: {len(guard_result.errors)}")
        
        # Test specific package
        pkg_target = TargetSpec(
            target="pkg:./cli/...",
            root=str(workspace_dir)
        )
...
async def test_typescript_project():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        core_package.write_text(json.dumps({
            "name": "@test/core",
            "version": "1.0.0"
        }, indent=2))
...
""")
        ...
        print(f"  fmt result: {'✅' if fmt_result.ok else '❌'} {fmt_result.backend_used}")
        
        build_result = await tools.build(target_spec, BuildArgs(opts={"dry_run": True}))
        print(f"  build result: {'✅' if build_result.ok else '❌'} {build_result.stdout}")
        
        lint_result = await tools.lint(target_spec, LintArgs(opts={"dry_run": True}))
        print(f"  lint result: {'✅' if lint_result.ok else '❌'} {lint_result.backend_used}")
...
""")
        ...
        for target, description in test_cases:
            ...
            print(f"  {description}: {'✅' if result.ok else '❌'} scope={len(result.scope_resolved)}")
...
""")
        ...
        edit_result = await tools.edit(target_spec, EditArgs(op="organize_imports", dry_run=True))
