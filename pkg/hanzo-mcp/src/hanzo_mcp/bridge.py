...
class RustBridge(McpBridge):
    ...
    def _run_rust_command(self, args: List[str]) -> str:
        ...
        try:
            result = subprocess.run(
                [str(self.rust_binary), "mcp"] + args, capture_output=True, text=True, check=True, timeout=30
            )
    ...
    async def execute(self, tool_name: str, params: Dict[str, Any]) -> Any:
        ...
        output = self._run_rust_command(["call", tool_name, "--params", json.dumps(params)])
        return json.loads(output)
...
class PythonBridge(McpBridge):
    ...
    def _register_all_tools(self):
        ...
        from .tools import ai, ast, git, edit, file, shell, search, browser, project
