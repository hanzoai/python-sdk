...
class HanzoMCPSwarmLauncher:
    ...
    def create_code_analysis_swarm(self) -> List[Any]:
        ...
        fs_agent = create_local_agent(
            ...
            tools=[
                create_tool(name="read_file", description="Read file contents", handler=read_file),
                create_tool(name="search_files", description="Search for files", handler=search_files)
            ],
        ...
        analysis_agent = create_local_agent(
            ...
            tools=[
                create_tool(name="analyze_code", description="Analyze code quality", handler=analyze_code)
            ],
            local_model="llama3.2"
        )
        ...
        refactor_agent = create_local_agent(
            ...
            tools=[
                create_tool(name="suggest_refactor", description="Suggest refactoring", handler=suggest_refactor)
            ],
            local_model="llama3.2"
        )
    ...
    def create_development_swarm(self) -> List[Any]:
        ...
        generator_agent = create_local_agent(
            ...
            tools=[
                create_tool(name="generate_code", description="Generate code", handler=generate_code)
            ],
            local_model="llama3.2"
        )
        ...
        test_agent = create_local_agent(
            ...
            tools=[
                create_tool(name="write_tests", description="Write test cases", handler=write_tests)
            ],
            local_model="llama3.2"
        )
        ...
        docs_agent = create_local_agent(
            ...
            tools=[
                create_tool(name="write_docs", description="Write documentation", handler=write_docs)
            ],
            local_model="llama3.2"
        )
    ...
    async def coordinate_swarms(self, complex_task: str):
        ...
        analysis_result = await self.launch_swarm(
            "analysis",
            f"Analyze the codebase for: {complex_task}"
        )
        
        # Phase 2: Development based on analysis
        dev_result = await self.launch_swarm(
            "development",
            f"Based on analysis, implement: {complex_task}"
        )
...
async def main():
    ...
    await launcher.launch_swarm(
        "analysis",
        "Find all authentication-related code and analyze security"
    )
    ...
    await launcher.launch_swarm(
        "development",
        "Create a new user registration system"
    )
    
    # Demo 3: Coordinated swarms
    print("\n\n📋 Demo 3: Coordinated Multi-Swarm Task")
    await launcher.coordinate_swarms(
    ...
    print(f"Inference engine: hanzo/net (local)")
    print(f"External API calls: 0")
    print(f"Privacy: 100% on-device execution")
