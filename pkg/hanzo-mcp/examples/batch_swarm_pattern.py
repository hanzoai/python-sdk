...
"""
            ...
            {
                "status": "success",
                "output": "Import blocks found in 15 files..."
            },
            {
                "status": "success",
                "output": "Directory structure analyzed..."
            }
        ],
        "completed": 4,
        "failed": 0,
...
async def execute_parallel_fixes(tasks: List[Dict], max_concurrency: int = 10):
    ...
    results = {
        "results": [
            {"task_index": i, "status": "completed", "agent_output": f"Fixed {tasks[i]['file']}"}
...
""")
    ...
    batch_config = {
        ...
        "invocations": [
            {
                "tool_name": "grep",
                "input": {"pattern": "OldAPI\\.\\w+\\(", "path": "/project", "include": "*.go"}
            },
            {
                "tool_name": "grep_ast",
