...
async def demo_unified_search():
    ...
    result = await search.run(pattern="UnifiedSearch", max_results_per_type=3)
    ...
    if result.data:
        ...
        print("   Note: Vector search would be used if embedding model is available")
    print()
    ...
    result = await search.run(pattern="async def", page_size=10, page=1)
...
async def demo_find_tool():
    ...
    result = await find.run(pattern="*.py", max_results=5)
    ...
    result = await find.run(pattern="*", min_size="10KB", max_results=5)
    ...
    result = await find.run(pattern="*", modified_after="1 hour ago", max_results=5)
