...
class HanzoMCPInstaller:
    ...
    def install_python_packages(self) -> bool:
        ...
        try:
            ...
            print(
                "❌ uv not found. Please install uv first: curl -LsSf https://astral.sh/uv/install.sh | sh"
            )
            return False
...
"""
    ...
    def create_config(self) -> bool:
        ...
        config = {
            ...
            "backends": {
                "go": {"fmt": "goimports", "local_prefix": "github.com/luxfi"},
                "ts": {"fmt": "prettier", "package_manager": "pnpm"},
                "py": {"fmt": "ruff", "test": "pytest"},
            },
            "guard_rules": [
                {
                    "id": "no_node_in_sdk",
    ...
    def check_mcp_server(self) -> bool:
        ...
        claude_config_file = (
            self.home / ".config" / "claude-desktop" / "claude_desktop_config.json"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Install Hanzo MCP Development Environment"
    )
    ...
    parser.add_argument(
        "--vscode", action="store_true", help="Install VS Code extension only"
    )
