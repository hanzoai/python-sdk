...
async def test_e2e_local_inference():
    ...
    result = await network.run(
        prompt="Echo the message 'Hello from E2E test'",
        initial_agent=echo_agent
    )
...
async def test_e2e_mcp_tools_with_local_llm():
    ...
    fs_agent = create_local_agent(
        ...
        tools=[
            ...
            create_tool(name="list_test_files", description="List files", handler=list_test_files)
        ],
        local_model="llama3.2"
    )
    ...
    result = await network.run(
        prompt="List the files in the test directory",
        initial_agent=fs_agent
    )
    assert result["success"]
    # The dummy model returns generic responses, so just check for success
    assert result["final_output"] is not None
