...
class TestHiddenFilePermissions:
    ...
    def test_dotfile_exclusion_behavior(self, tool_helper, temp_dir: str):
        ...
        assert manager.is_path_allowed(problem_path), "Should allow .github-workflow-example.yml"
        ...
        assert manager.is_path_allowed(actual_github_dir), "Should allow actual .github directory"
    ...
    def test_path_component_matching(self, tool_helper, temp_dir: str):
        ...
        for path in full_component_paths:
            assert not manager.is_path_allowed(path), f"Should exclude full component: {path}"

        # Test partial component paths (should be allowed)
        for path in partial_component_paths:
            assert manager.is_path_allowed(path), f"Should allow partial component: {path}"
    ...
    def test_wildcard_patterns(self, tool_helper, temp_dir: str):
        ...
        for path in wildcard_matches:
            assert not manager.is_path_allowed(path), f"Should exclude wildcard match: {path}"
