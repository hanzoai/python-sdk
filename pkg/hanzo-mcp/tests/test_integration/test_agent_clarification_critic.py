...
async def test_clarification_limits(test_project, mock_context):
    ...
    with patch("litellm.completion") as mock_completion:
        ...
        result = await agent.call(
            mock_context,
            prompts="Test clarification limits"
        )
...
async def test_critic_limits(test_project, mock_context):
    ...
    with patch("litellm.completion") as mock_completion:
        ...
        mock_completion.side_effect = [
            ...
            Mock(choices=[Mock(message=Mock(
                content=None,
                tool_calls=[Mock(function=Mock(name="critic", arguments=review_args), id="call_1")]
            ))]),
            # Second review
            Mock(choices=[Mock(message=Mock(
                content=None,
                tool_calls=[Mock(function=Mock(name="critic", arguments=review_args), id="call_2")]
            ))]),
            # Third review (should fail)
            Mock(choices=[Mock(message=Mock(
                content=None,
                tool_calls=[Mock(function=Mock(name="critic", arguments=review_args), id="call_3")]
            ))]),
            # Final response
            Mock(choices=[Mock(message=Mock(
        ...
        result = await agent.call(
            mock_context,
            prompts="Test critic review limits"
        )
