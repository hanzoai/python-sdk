...
@pytest.mark.asyncio
async def test_swarm_parallel_file_edits(temp_project, mock_mcp_context, monkeypatch):
...
@pytest.mark.asyncio
async def test_swarm_handles_errors(temp_project, mock_mcp_context, monkeypatch):
    ...
    assert "❌" in result  # Error indicator
    assert "✅" in result  # Success indicator


@pytest.mark.asyncio
async def test_swarm_respects_concurrency_limit(temp_project, mock_mcp_context, monkeypatch):
