...
class TargetResolver:
    ...
    def resolve(self, target: str, language: str = "auto", root: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _resolve_file(self, file_path: str, language: str, root: Optional[str]) -> Dict[str, Any]:
        """Resolve single file"""
    ...
    def _resolve_dir(self, dir_path: str, language: str, root: Optional[str]) -> Dict[str, Any]:
    ...
    def _resolve_package(self, pkg_spec: str, language: str, root: Optional[str]) -> Dict[str, Any]:
    ...
    def _resolve_changed(self, language: str, root: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Make paths absolute
            root_path = Path(workspace["root"])
            absolute_files = [str(root_path / f) for f in changed_files if (root_path / f).exists()]
...
class SimpleTools:
    ...
    def fmt(self, target: str, language: str = "auto", dry_run: bool = False, opts: Optional[Dict] = None) -> ToolResult:
    ...
    def test(self, target: str, language: str = "auto", dry_run: bool = False, opts: Optional[Dict] = None) -> ToolResult:
    ...
    def edit(self, target: str, op: str, language: str = "auto", dry_run: bool = False, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if op == "organize_imports":
                ...
                action_desc = "Would organize imports" if dry_run else "Organized imports"
            elif op == "rename":
                touched_files = [kwargs.get("file", "")] if not dry_run and kwargs.get("file") else []
                new_name = kwargs.get("new_name", "NewName")
                action_desc = f"Would rename to {new_name}" if dry_run else f"Renamed to {new_name}"
            else:
    ...
    def build(self, target: str, language: str = "auto", dry_run: bool = False, opts: Optional[Dict] = None) -> ToolResult:
    ...
    def lint(self, target: str, language: str = "auto", dry_run: bool = False, opts: Optional[Dict] = None) -> ToolResult:
    ...
    def guard(self, target: str, rules: List[Dict], language: str = "auto", dry_run: bool = False) -> ToolResult:
        ...
        try:
            ...
            for rule in rules:
                ...
                if rule.get("type") == "import" and rule.get("forbid_import_prefix"):
                    ...
                    violations.append(f"Found forbidden import '{rule['forbid_import_prefix']}' in files matching {rule['glob']}")
...
""")
        ...
        print(f"  ✅ go.work detected correctly")
        ...
        for target, description in test_targets:
            try:
                resolved = tools.target_resolver.resolve(target, root=str(workspace_dir))
                print(f"  {description}: {len(resolved['paths'])} files, type: {resolved['type']}")
            except Exception as e:
        ...
        # Test each tool
        test_cases = [
            ("fmt", tools.fmt, {"target": "ws", "dry_run": True, "opts": {"local_prefix": "github.com/luxfi"}}),
            ("edit", tools.edit, {"target": "file:" + str(api_main), "op": "organize_imports", "dry_run": True}),
            ...
            ("guard", tools.guard, {
                "target": "ws",
                "rules": [
                    {"id": "no-net-http", "type": "import", "glob": "api/*.go", "forbid_import_prefix": "net/http"}
                ],
                "dry_run": True
            })
        ]
        ...
        for tool_name, tool_func, kwargs in test_cases:
            try:
                ...
                print(f"  {tool_name}: {status} {result.backend_used} - {result.stdout}")
            except Exception as e:
                print(f"  {tool_name}: ❌ {e}")
        ...
        guard_result = tools.guard(
            target="ws",
            rules=[{"id": "no-http", "type": "import", "glob": "**/*.go", "forbid_import_prefix": "net/http"}],
            dry_run=True
        )
        print(f"  4. guard check: {'✅' if guard_result.ok else '❌'} ({len(guard_result.errors)} violations)")
    
    print(f"\n🎉 Demonstration complete!")
    print(f"\n✨ Key Features Demonstrated:")
    print(f"  • Workspace detection with go.work priority")
    print(f"  • Target resolution (file:, dir:, pkg:, ws, changed)")
    print(f"  • Backend selection (language-specific tools)")
    print(f"  • All 6 universal tools (edit, fmt, test, build, lint, guard)")
    print(f"  • Tool composition workflows") 
    print(f"  • Consistent input/output schemas")
    print(f"  • Dry-run support")
